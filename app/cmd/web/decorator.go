...
type GinUserRoute func(c *gin.Context, db *gorm.DB, user *model.User)
type GinDomainRoute func(c *gin.Context, db *gorm.DB, user *model.User, domain *model.Domain)
...
func decorateRequireDomain(wrapped GinDomainRoute) GinRoute {
	return func(c *gin.Context, db *gorm.DB) {
		decorateRequireUser(func(c *gin.Context, db *gorm.DB, user *model.User) {
			view := View{c}
